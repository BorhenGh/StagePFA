...
function init(botLogoPath) {
    ...
    //--------------------------- Important Variables----------------------------
     botLogoPaths = "chatbot.png"
    
    //--------------------------- Chatbot Frontend -------------------------------
    const chatContainer = document.getElementById("chat-container");
    
    template = ` <button class='chat-btn'><img src='http://127.0.0.1:8001/chatbot/chatbot.png' class="fx animated fadeInLeft" data-animate="fadeInLeft" width="30px"></button>
    
    <div class='chat-popup fx animated  pulse '>
    
        <div class='chat-header'>
            <div class='chatbot-img'>
                <img src='${botLogoPath}' alt='Chat Bot image' class='bot-img'> 
            </div>
            <h3 class='bot-title'>DoctorSina Bot</h3>
            <button class = "expand-chat-window" ><img src="http://127.0.0.1:8001/chatbot/fullscreen.png" class="material-icon" width="30px"></button>
        </div>
    
        <div class="chat-area">
            <div  id="bot-msg-animate" class='bot-msg' >
            ...
            <!-- <div id="bot-msg-animate"  class='user-msg'>
                <span class='msg'>Hi, How can i help you?</span>
            </div> -->
        ...
        <div class='chat-input-area'>
            ...
            <button class='chat-submit'><i class="fa fa-paper-plane" ></i></button>
        </div>
    ...
    chatBtn.addEventListener("click", () => {
        ...
        if (chatPopup.style.display == "none" && mobileDevice) {
            ...
            chatInput.focus();
            chatBtn.innerHTML = `<img src="http://127.0.0.1:8001/chatbot/chatbot_close.png" class="fx animated fadeInLeft" data-animate="fadeInLeft" width="30px">`
        } else if (mobileDevice) {
            chatPopup.style.display = "none"
            chatBtn.innerHTML = `<img src="http://127.0.0.1:8001/chatbot/chatbot.png" class="fx animated fadeInLeft" data-animate="fadeInLeft" data-animation-delay="50" width="30px">`
        } else {
            mobileView()
        }
    })
    ...
    expandWindow.addEventListener("click", (e) => {
        ...
        if (expandWindow.innerHTML == '<img src="http://127.0.0.1:8001/chatbot/fullscreen.png" class="material-icon" width="30px">') {
            expandWindow.innerHTML = `<img src = "http://127.0.0.1:8001/chatbot/fullscreen_close.png" class = 'material-icon' width="30px">`
            ...
            expandWindow.innerHTML = `<img src = "http://127.0.0.1:8001/chatbot/fullscreen.png" class = "material-icon" width="30px">`
    ...
    ****************************************************************/
    ...
    setTimeout(function() {
        if (val.length < 1) {
            ...
            $(BotResponse).appendTo('.chat-area').hide().fadeIn(100);
            scrollToBottomOfResults();
            chatInput.focus();
            ...
            for (i = 0; i < val.length; i++) {
                ...
                if (val[i].hasOwnProperty("text")) {
                    var matches = val[i].text.match(/\bhttps?:\/\/\S+/gi);
                    var replace = `<a class="bot-link" href="${matches}">Link</a>`;
                    var BotResponse = `<div class='bot-msg '><img class='bot-img' src ='${botLogoPath}' /><span class='msg' >${val[i].text.replace(matches, replace)} </span></div>`;
                    const element = document.querySelector('.bot-msg');
                ...
                if (val[i].hasOwnProperty("image")) {
                    var BotResponse = `<div  class='bot-msg'>` + `<img class='bot-img' title='what' src ='${botLogoPath}' />` +
